...
//		デバッグ用："C:\Program Files\HMS\Ixxat VCI\sdk\vci\lib\x32"
//		リリース用："C:\Program Files\HMS\Ixxat VCI\sdk\vci\lib\x32\release"
//
//========================================
//使用方法
//========================================
...
int32_t CIxxatV2::OnOpenInterface(int32_t nDeviceNum)
	...
	//デバイスマネージャを使用してデバイス列挙を取得する
	IVciEnumDevice* pEnum = NULL;
	hResult = pDevMgr->EnumDevices(&pEnum);

	//デバイス列挙を使用してデバイス数を取得する
	int32_t nCount = 0;	//
	VCIDEVICEINFO sInfo;
	while (hResult == VCI_OK)
		{
		//デバイス列挙を使用して、デバイス情報を(先頭から)順に取得する
		hResult = pEnum->Next(1,&sInfo,NULL);
		if (hResult == VCI_OK)
			{
...
int32_t CIxxatV2::OnCanRecv(uint32_t* pCanID,uint8_t* pData8)
	{
	//概要
...
uint32_t CIxxatV2::AddCanMsg(PCANMSG pMsg,int nCount /* 1 */)
	...
	while(nCount)
		...
		if (pMsg->uMsgInfo.Bytes.bType == CAN_MSGTYPE_DATA)
			...
			//For DEBUG
			if(DEBUGPRINT)
				debugprint(false,pMsg->dwMsgId,pMsg->abData);
